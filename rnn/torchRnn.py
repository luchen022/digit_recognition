...
class SentimentRNN(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim):
        ...
        # RNN：输入向量序列，每个时间步更新隐藏状态
        self.rnn = nn.RNN(embed_dim, hidden_dim, batch_first=True,
                          num_layers=2, dropout=0.3, nonlinearity='tanh')
    ...
    def forward(self, x):
        ...
        _, h_n = self.rnn(emb)            # h_n: (num_layers, batch, hidden_dim)
...
def train():
    ...
    torch.save({'model_state': model.state_dict(), 'vocab': vocab}, 'sentiment_rnn.pth')
    print("模型已保存到 sentiment_rnn.pth")

    plt.plot(loss_history)
    plt.title("ChnSentiCorp Training Loss (RNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.grid(True)
    plt.show()
